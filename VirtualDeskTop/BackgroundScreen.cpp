...
void BackgroundScreen::initScreen(float x, float y, float z, float w, float h){
	...
	temp.Pos = Vector3f(ir, y + h, z); temp.U = 1.0f; temp.V = 1.0f;
	Vertices.push_back(temp);
	temp.Pos = Vector3f(ir, y, z); temp.U = 1.0f; temp.V = 0.0f;
	Vertices.push_back(temp);

	for (i = 1; i <= division; i++) {
		ir -= rs;
		temp.Pos = Vector3f(ir, y, z); temp.U = 1.0f - ts*i; temp.V = 0;
		Vertices.push_back(temp);
		temp.Pos = Vector3f(ir, y + h, z); temp.U = 1.0f - ts*i; temp.V = 1.0f;
		Vertices.push_back(temp);
	}
}